...
def main():
    ...
    workdir = Path().cwd()
    runs_range = (0, 1)
    experiments_dir = Path(workdir/f"experiments/{targeted_num_threads}-thread-papi-naive")
    ...
    for benchmark in benchmarks:
        ...
        benchmark_binary = Path(workdir/f"cbuild/llvm-exec/papi_naive_{targeted_num_threads}_thread_exe_{benchmark}/papi_naive_{targeted_num_threads}_thread_exe_{benchmark}")
        if not benchmark_binary.exists():
            print(f"Binary for {benchmark} does not exist")
            print(f"binary path: {benchmark_binary}")
            exit(1)
