...
def run_this(run_ball):
    ...
    command = ["taskset", "-c", f"{core_num}" ] + cmd
...
def main():
    cores = [i for i in range(8) if i%2 == 0]
    ...
    env["LD_LIBRARY_PATH"] = "/home/studyztp/compiler/llvm-dir/lib/x86_64-unknown-linux-gnu;"
    ...
    workdir = Path().cwd()
    runs_range = (0, 5)
    experiments_dir = Path(workdir/f"experiments/{num_threads}-thread-papi-nuggets/liberty-experiments")
    experiments_dir.mkdir(parents=True, exist_ok=True)

    rid_file_dir = Path(workdir/f"experiments/info/random-selections/{num_threads}/{size}")
    ...
    for benchmark in benchmarks:
        ...
        for rid in all_benchmark_rids:
            ...
            benchmark_binary = Path(workdir/f"cbuild/llvm-exec/{num_threads}_thread_time_nugget_{grace_perc}_{opt_level}_exe_{benchmark}_{rid}/{num_threads}_thread_time_nugget_{grace_perc}_{opt_level}_exe_{benchmark}_{rid}")
